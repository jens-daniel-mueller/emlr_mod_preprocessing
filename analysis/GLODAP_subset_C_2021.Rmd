---
title: "GLODAP subset constant climate C"
...
Here we subset cmorized (1x1) data from the model with constant forcing, according to the presence of GLODAP observations in a previously cleaned file.
...
```{r subset_cmorized_constant_forcing_data, eval=FALSE}
...
for (i_year in years) {
  ...
  for (i_variable in variables) {
    ...
    ggsave(
      paste(
        path_preprocessing,
        "regular_subset_distribution_runC_2021/",
        i_variable,
        "_",
        i_year,
        ".png",
...
GLODAP %>%
  write_csv(
    paste(
      path_preprocessing,
      "GLODAPv2.2021_preprocessed_model_runC_raw_subset.csv",
      sep = ""
    )
  )
...
GLODAP %>%
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2021_preprocessed_model_runC_both.csv",
                  sep = ""))
...
GLODAP %>%
  select(-row_number) %>% 
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2021_preprocessed_model_runC_final.csv",
                  sep = ""))
...
variable_data_tibble <-
  read_ncdf(paste(path_cmorized, 
    "dissic_CESM-ETHZ_C_1_gr_2019.nc",
    sep = ""
  )) %>%
  as_tibble()
...
GLODAP_cmorized <- read_csv(paste(path_preprocessing,
                                  "GLODAPv2.2020_preprocessed_model_runC_both.csv",
                                  sep = ""))
