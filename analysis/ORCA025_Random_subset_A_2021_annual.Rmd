...
model <-
  read_ncdf(paste(path_cmorized, 
    "dissic_ORCA025-GEOMAR_A_1_gr_19800101_19891231-v20210204.nc",
    sep = ""
  )) %>%
  as_tibble() %>%
  filter(!is.na(dissic))
...
model_resample %>%
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2021_preprocessed_model_runA_random_subset_grid.csv",
                  sep = ""))
...
GLODAP_depth <- GLODAP %>%
  mutate(depth = as.numeric(as.character(cut(depth,
                     ...
                     labels = unique(model_resample_depth_average$depth)[1:45])))) %>% 
  count(depth, year)
...
resample_lat %>%
  write_csv(paste(path_preprocessing,
                 "GLODAPv2.2021_preprocessed_model_runA_random_subset_lat.csv",
                 sep = ""))
