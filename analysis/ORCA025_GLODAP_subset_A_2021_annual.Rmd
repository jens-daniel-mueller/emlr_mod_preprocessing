...
path_cmorized <-
  "/nfs/kryo/work/updata/reccap2/Models/3D_ALL/ORCA025-GEOMAR_3D_ALL_v20210204/"
...
for (i_variable in variables) {
  ...
  for (i_period in periods) {
    ...
    # read in data
    variable_data <-
      read_ncdf(paste(path_cmorized, file,
                      sep = ""),
                make_units = FALSE)
...
GLODAP_NA_filled %>%
  write_csv(
    paste(
      path_preprocessing,
      "GLODAPv2.2021_preprocessed_model_runA_both_NA_filled.csv",
      sep = ""
    )
  )
...
GLODAP_NA_filled %>%
  select(-row_number) %>% 
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2021_preprocessed_model_runA_final_NA_filled.csv",
                  sep = ""))
...
variable_data_tibble <-
  read_ncdf(paste(path_cmorized, 
    "dissic_ORCA025-GEOMAR_A_1_gr_20100101_20181231-v20210204.nc",
    sep = ""
  )) %>%
  as_tibble()
