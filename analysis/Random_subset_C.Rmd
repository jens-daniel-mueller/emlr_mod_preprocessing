---
title: "Random subset constant C"
...
Here we randomly subset cmorized (1x1) model with constant forcing, according to the total number of GLODAP observations for the whole period from a previously cleaned file. The number for the annual subset remains the same for each year, which could be expressed by the total number of observations divided by number of years.
...
```{r random_subset_cmorized_constant_forcing_data}

# read in a random model
model <-
  read_ncdf(paste(path_cmorized, "dissic_CESM-ETHZ_C_1_gr_1982.nc", sep = "")) %>%
  as_tibble() %>%
  drop_na()
...
# Resample equal to area

The previous sampling was performed randomly across a 1x1 horizontal grid, resulting in a higher number of samples per surface area at higher latitude. This bias is corrected by resampling with a latitude dependend weight.

```{r resample_with_less_high_lat_points}
