...
# Zonal mean section for Cant in 2007
cant_annual_zonal <- cant_annual %>%
  filter(year == 2007) %>%
  select(-c(lon, tco2_A, tco2_B, year)) %>%
...
for (i_inventory_depth in params_global$inventory_depths) {
  ...
  dcant_gruber_inv_temp <- dcant_gruber_temp %>%
    group_by(lon, lat, basin_AIP) %>%
    summarise(dcant_inv = sum(dcant_layer_inv, na.rm = TRUE) / 1000) %>%
    ungroup()
...
rm(dcant_gruber_inv_temp, dcant_gruber_temp, depth_level_volume)
...
p_map_climatology(df = cant_annual_year, var = "cant", col = "divergent")
...
p_map_climatology(df = dcant_gruber_round, var = "dcant", col = "divergent")
