...
#  i <- 1
  ...
  for (i_basin_AIP in unique(GLODAP_cmorized_var_zonal$basin_AIP)) {
   # i_basin_AIP = "Atlantic"
    ...
    lat_max <- max(GLODAP_cmorized_var_zonal$lat)
    lat_min <- min(GLODAP_cmorized_var_zonal$lat)
    print(
      GLODAP_cmorized_var_zonal %>%
        ggplot(aes(lat, depth, z = offset)) +
        geom_contour_filled() +
        scale_fill_viridis_d(name = "offset") +
