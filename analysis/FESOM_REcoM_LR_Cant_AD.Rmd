...
path_preprocessing <-
  paste(path_root, "/model/preprocessing/RECCAP2-ocean_models/cant_AD/", sep = "")
...
```{r read_cmorized_Runs_A_D, eval=FALSE}
...
for (i_model in models) {
  ...
  AD_annual <- AD_annual %>% pivot_wider(
    names_from = model_ID,
    values_from = dissic
  )
...
```{r zonal_mean_section_dcant, fig.asp=1.5}
...
```{r zonal_mean_section_tcant, fig.asp=1.5}
...
```{r dcant_column_inventory, fig.asp=1.5}
