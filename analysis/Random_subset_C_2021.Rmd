---
title: "Random subset constant C 2021"
...
model <-
  tidync(
    paste(
      path_cmorized,
      "dissic_CESM-ETHZ_C_1_gr_1980-2018_v20210607.nc",
      sep = ""
    )
  ) %>%
  hyper_tibble() %>%
...
for (i_variable in variables) {
 # i_variable <- variables[2]
  ...
  variable_data_tibble <- variable_data_tibble %>%
    mutate(year = 1980 + floor(time_ann / 365)) %>%
    select(-time_ann)
...
model_resample %>%
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2021_preprocessed_model_runC_random_subset_grid_ann.csv",
                  sep = ""))
...
resample_lat %>%
  write_csv(paste(path_preprocessing,
                 "GLODAPv2.2021_preprocessed_model_runC_random_subset_lat_ann.csv",
                 sep = ""))
