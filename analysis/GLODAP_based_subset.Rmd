...
GLODAP %>%
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2020_preprocessed_with_model_runA.csv",
                  sep = ""))
...
GLODAP %>%
  write_csv(paste(path_preprocessing,
                  "GLODAPv2.2020_preprocessed_model_runA.csv",
                  sep = ""))
...
# select GLODAP data for 2019
  ...
  # create month x lat x lon grid of observations
  grid_subset <- GLODAP_2019 %>%
    select(month, lat, lon) %>%
    unique()
  
  # create month x lat x lon x depth grid of observations
  obs_depth <- GLODAP_2019 %>%
    select(month, lat, lon, depth) %>%
    unique()
  
  # read in cmorized tco2 output in year 2019
  variable_data_tibble <-
      read_ncdf("/nfs/kryo/work/loher/CESM_output/RECCAP2/cmorized_Dec2020/split_monthly/dissic_CESM-ETHZ_A_1_gr_1982.nc") %>%
      as_tibble()
...
ggplot() +
  ...
  geom_point(
    data = model_obs_interpo %>%
      filter(lat == -47.5, lon == 306.5, month == 4),
...
GLODAP_max_depth <- GLODAP %>%
  ...
  group_by(lon, lat) %>%
  mutate(depth = max(depth)) %>%
  rename(depth_GLODAP = depth) %>%
  unique()
...
model_max_depth <- variable_data_tibble %>%
  ...
  group_by(lon, lat) %>%
  mutate(depth = max(depth)) %>%
  rename(depth_model = depth) %>%
  unique()

GLODAP_model_depth_offset <-
  left_join(GLODAP_max_depth, model_max_depth) %>%
  mutate(offset = depth_GLODAP - depth_model)

map +
  geom_point(
    data = GLODAP_model_depth_offset %>%
      filter(offset > 0),
    aes(lon, lat, col = "Obs beyond model"),
    size = 1
  ) +
  geom_point(
    data = GLODAP_model_depth_offset %>%
      filter(offset < 0),
    aes(lon, lat, col = "Obs below model"),
    size = 1
  ) +
  labs(
