...
for (i_model in models) {
  ...
  # Remove 3D_all from model name
  i_model <- str_remove(i_model, "3D_ALL_")
  i_model <- str_remove(i_model, "3D_all_")
...
```{r read_GLODAP_with_cmorized_subset_file, eval=FALSE}

GLODAP_cmorized <- read_csv(
  paste(
    path_preprocessing,
...
```{r calc_revelle_factors, eval=FALSE}
...
```{r GLODAP_vs_cmorized_subset_comparison, eval=FALSE}
