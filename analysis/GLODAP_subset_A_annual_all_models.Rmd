...
path_preprocessing <-
  paste0(path_root, "/model/preprocessing/GLODAP_subset_A_annual_all_models/")
```
...
for (i_model in models) {
  ...
  variables_available <- str_split(variables_available, pattern = "_", simplify = TRUE)[,1]
  variables_available <- variables_available[variables_available %in% variables]
  ...
  for (i_variable in variables_available) {
    ...
    file <- file[str_detect(file, pattern = paste0("_", model_ID, "_"))]
    ...
    variable_data_tibble <- variable_data_tibble %>%
      ...
      select(-time)
    # unique(variable_data_tibble$year)
    ...
    for (i_year in years) {
      ...
      ggsave(
        paste(
          path_preprocessing,
          "regular_subset_distribution_runA_2021_ann/",
          i_model,
          i_variable,
          "_",
          i_year,
...
for (i_model in models) {
  ...
  GLODAP_single <- GLODAP_single %>% 
    mutate(model = str_replace(i_model, "_3D_ALL_|_3D_all_", "|"))
...
GLODAP %>%
  write_csv(
    paste(
      path_preprocessing,
      "all_models_GLODAPv2.2021_preprocessed_model_runA_both_ann.csv",
      sep = ""
    )
  )
...
GLODAP_NA_filled %>%
  write_csv(
    paste(
      path_preprocessing,
      "all_models_GLODAPv2.2021_preprocessed_model_runA_both_NA_filled_ann.csv",
      sep = ""
    )
  )
...
GLODAP %>%
  ...
  write_csv(paste(path_preprocessing,
                  "all_models_GLODAPv2.2021_preprocessed_model_runA_final_ann.csv",
                  sep = ""))

GLODAP_NA_filled %>%
  select(-row_number) %>% 
  write_csv(paste(path_preprocessing,
                  "all_models_GLODAPv2.2021_preprocessed_model_runA_final_NA_filled_ann.csv",
                  sep = ""))
...
#  i <- 12
  ...
  pdf(
    file = paste0(path_preprocessing, "models_vs_GLODAP/", obs_var[i],
